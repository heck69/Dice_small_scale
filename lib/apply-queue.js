...
function createApplyQueue(supabase) {
  async function enqueueApplyJob({ clientId, telegramChatId, url, jobId = null }) {
    ...
    const { data: activeClientJob, error: activeClientError } = await supabase
      .from('apply_queue')
      .select('id, status')
      .eq('client_id', clientId)
    ...
    if (activeClientError) throw new Error(`Client queue status lookup failed: ${activeClientError.message}`);
    ...
    if (activeClientJob && activeClientJob.url !== url) {
      return { row: activeClientJob, created: false, activeClientJob: true };
    }
  ...
  async function claimNextJob(workerId) {
    const { data, error } = await supabase.rpc('claim_apply_queue_job', {
  ...
  async function hasActiveOrFinishedQueueItem(clientId, url) {
    ...
    return ['queued', 'running', 'completed'].includes(data.status);
  }
